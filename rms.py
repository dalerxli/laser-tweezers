...
parser.add_argument('--nofilter', dest='filter_on', action='store_false',
                    help='Highpass filter off (default)')
...
def dict_to_df_rms(d, cnames):
    ...
    return df
def scan_transformation_rms(infile, scan_name, output_dict, col=2, 
                            filter_hp=False, export_signal=False):
    ...
    if len(infile) > 0: # if there is at least 1 force-save*.txt
        ...
        if export_signal == True:
            ...
            for i in range(len(infile)): # for each force-save*.txt
                ...
                if filter_hp == True:
                    sig = butter_highpass_filter(sig, highcut=args.cf_low,
                                                 fs=p['fs'], order=3)

                rms = rms_calc(sig)
                output_dict[colnames[i]] = rms
                sig_d[colnames[i]] = sig
                logger.info(' '.join(("Finished:", colnames[i])))
            return output_dict, sig_d
...
def rms_calc(sig):
    ...
    return rms
def main_rms_run_basic(p, filter_hp=False, export_signal=False):
    rms_d = {}
    ...
    for subdir, dirs, files in os.walk(p['rootpath']):
        ...
        if export_signal == True:
            rms_d, sig_d = scan_transformation_rms(glob('force-save*.txt'), scan_name,
                                            rms_d, filter_hp=filter_hp,
                                            export_signal=True)
        else: # Regular/default b/h
    ...
    def export(d, dv):
        """Hack in signal output for comparison test"""
        df = dict_to_df_rms(d, cnames=['Scan_Name', dv])
    ...
    if export_signal == True:
        dv = 'sig_filtered' if filter_hp == True else 'sig'
        df = dict_to_df_rms(sig_d, cnames=['Scan_Name', dv])
        rootpathname = '/'.join((p['rootpath'], p['rootpath'].split('/')[-1]))
        outfile = '_'.join((rootpathname, dv.lower() + '.csv'))
        logger.info('Exporting %s csv' %dv)
        df.to_csv(outfile, index=False)
