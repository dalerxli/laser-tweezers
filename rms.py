...
def get_params(f):
    ...
    return t, fs, date
def rms_calc(sig):
    # Maybe add a filter filter here?
    sig = sig - np.mean(sig) #baseline corrected sig
...
def scan_transformation_rms(infile, scan_name, output_dict, col=2, filter_hp=False):
    ...
    if len(infile) > 0: # if there is at least 1 force-save*.txt
        ...
        return output_dict
def scan_transformation_rms_filtercomp(infile, scan_name, output_dict, col=2, filter_hp=True):
    """ Assume filter is true and we want to compare filtered and unfiltered
    rms.
    Run the above functions and store psd for each scan in a dict (psd_d)
    Also store extracted raw signal in a dict
    ...
    if len(infile) > 0: # if there is at least 1 force-save*.txt
        ...
        for i in range(len(infile)): # for each force-save*.txt
            ...
            ## Run highpass filter, setup comparison
            if filter_hp == True:
                sig_filt = butter_highpass_filter(sig, highcut=args.cf_low, fs=p['fs'], order=3)
                rms_filt = rms_calc(sig_filt)
                # store in a sub dict
                output_dict[colnames[i]] = {'RMS' : rms,
                                            'RMS_filtered' : rms_filt}
